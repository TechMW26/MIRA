...
const POLLINATIONS_ORIGIN = String(process.env.POLLINATIONS_API_URL || 'https://gen.pollinations.ai')
  .trim()
  .replace(/\/+$/, '');
const POLLINATIONS_CHAT_URL = `${POLLINATIONS_ORIGIN}/v1/chat/completions`;
...
export function buildPollinationsVisionPayload({ prompt = '', images = [] } = {}) {
  ...
  return {
    model: String(process.env.POLLINATIONS_VISION_MODEL || 'openai').trim(),
    messages: [{ role: 'user', content: [{ type: 'text', text: cleanPrompt }, ...imageParts] }],
  };
}
...
export async function generatePollinationsVisionAnalysis({ prompt, images, signal } = {}) {
  const payload = buildPollinationsVisionPayload({ prompt, images });
...
export async function generateVisionAnalysis({ prompt, images, signal } = {}) {
  // Validate once up front, then run Pollinations as the vision provider.
  buildPollinationsVisionPayload({ prompt, images });
  return await generatePollinationsVisionAnalysis({ prompt, images, signal });
}
