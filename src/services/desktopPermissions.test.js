...
test('desktop permissions are ordered sequentially for each operating system', () => {
  assert.deepEqual(
    ...
    ['notifications', 'microphone', 'camera', 'location', 'screen-capture', 'accessibility', 'full-disk-access'],
  );
  assert.deepEqual(
    getDesktopPermissionSequence('win32').map((item) => item.id),
    ['notifications', 'microphone', 'camera', 'location', 'screen-capture', 'full-disk-access'],
  );
});
