...
import { X, Plus, Search, BookMarked, Trash2, Send, Star, Edit3 } from 'lucide-react';
...
export default function PromptLibrary({ onUsePrompt, onClose }) {
  ...
  useEffect(() => {
    if (!user) return;
    const promptsRef = ref(db, `prompts/${user.uid}`);
    onValue(promptsRef, snap => {
      const items = [];
      snap.forEach(child => items.push({ id: child.key, ...child.val() }));
      setSaved(items.reverse());
    });
    return () => off(promptsRef);
  }, [user]);

  async function savePrompt() {
    if (!user || !newTitle.trim() || !newPrompt.trim()) return;
    const promptRef = push(ref(db, `prompts/${user.uid}`));
    await set(promptRef, { title: newTitle.trim(), prompt: newPrompt.trim(), createdAt: Date.now() });
    setNewTitle('');
    setNewPrompt('');
    setAdding(false);
  }
  ...
  function openContextModal(title, prompt) {
    setContextModal({ title, prompt });
    setEditedPrompt(prompt);
    setExtraContext('');
  }

  function handleSendWithContext() {
  ...
  const filtered = (tab === 'featured' ? FEATURED : saved).filter(prompt =>
    prompt.title.toLowerCase().includes(search.toLowerCase()) || prompt.prompt.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <>
      <div className="flex flex-col h-full w-full">
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-2">
          {filtered.map((prompt, index) => (
            <div key={prompt.id || index} className="rounded-xl p-3 group" style={{ background: 'var(--glass-bg)', border: '1px solid var(--border)' }}>
              <div className="flex items-start justify-between gap-2 mb-1">
                <span className="text-xs font-semibold" style={{ color: 'var(--text-primary)' }}>{prompt.title}</span>
                {prompt.category && <span className="text-[9px] px-1.5 py-0.5 rounded-md flex-shrink-0" style={{ background: 'var(--accent-glow)', color: 'var(--accent)' }}>{prompt.category}</span>}
              </div>
              <p className="text-[11px] mb-2 line-clamp-2" style={{ color: 'var(--text-tertiary)' }}>{prompt.prompt}</p>
              <div className="flex gap-1">
                <button onClick={() => openContextModal(prompt.title, prompt.prompt)}
                  className="flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] font-medium transition-all hover:opacity-80"
                  style={{ background: 'var(--accent)', color: '#fff' }}>
                  <Send size={9} /> Use
                </button>
                {tab === 'saved' && (
                  <button onClick={() => deletePrompt(prompt.id)} className="p-1 rounded-lg hover:opacity-70" style={{ color: 'var(--text-tertiary)' }}>
                    <Trash2 size={11} />
                  </button>
                )}
                {tab === 'featured' && (
                  <button onClick={() => { setNewTitle(prompt.title); setNewPrompt(prompt.prompt); setAdding(true); setTab('saved'); }}
          ...
          <div className="p-2 flex-shrink-0" style={{ borderTop: '1px solid var(--border)' }}>
            {adding ? (
              <div className="space-y-2">
                <input value={newTitle} onChange={event => setNewTitle(event.target.value)} placeholder="Prompt title"
                ...
                <textarea value={newPrompt} onChange={event => setNewPrompt(event.target.value)} placeholder="Prompt text..." rows={3}
      ...
      {contextModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setContextModal(null)}>
          <div
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: 'var(--border)' }}>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold">{contextModal.title}</span>
              </div>
              <button onClick={() => setContextModal(null)} className="p-1 rounded hover:opacity-70">
            ...
            <div className="p-4 space-y-4">
              <div>
                <label className="text-[11px] font-medium mb-1 block" style={{ color: 'var(--text-secondary)' }}>Prompt</label>
                <textarea
                  value={editedPrompt}
              ...
              <div>
                <label className="text-[11px] font-medium mb-1 block" style={{ color: 'var(--text-secondary)' }}>
                  Additional Context <span className="font-normal opacity-60">(optional)</span>
                </label>
                <textarea
                  value={extraContext}
                  onChange={event => setExtraContext(event.target.value)}
                  placeholder="Paste code, add extra instructions, mention constraints..."
                  rows={6}
                  className="w-full text-xs px-3 py-2 rounded-xl outline-none resize-y min-h-[80px]"
                  style={{ background: 'var(--hover-bg)', color: 'var(--text-primary)', border: '1px solid var(--border)' }}
                />
              </div>
              ...
              <div className="flex gap-2 pt-1">
                <button
                  ...
                  style={{ background: 'var(--hover-bg)', color: 'var(--text-secondary)', borderColor: 'var(--border)' }}
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-2 rounded-xl text-xs font-medium flex items-center justify-center gap-1.5 transition-opacity hover:opacity-90 disabled:opacity-50"
                  style={{ background: 'var(--accent)', color: '#fff' }}
                >
                  <Send size={11} /> Send Prompt
                </button>
              </div>
            </div>
          </div>
