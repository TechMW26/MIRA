...
export default function PromptLibrary({ onUsePrompt, onClose }) {
  ...
  // New modal state
  const [contextModal, setContextModal] = useState(null); // { title, prompt }
  ...
  // Open the context popup for any prompt
  const openContextModal = (title, prompt) => {
    setContextModal({ title, prompt });
    setEditedPrompt(prompt);
    setExtraContext('');
  };

  // Send final combined prompt
  const handleSendWithContext = () => {
    if (!contextModal) return;
    const final = extraContext.trim()
      ? `${editedPrompt.trim()}\n\nAdditional Context:\n${extraContext.trim()}`
      : editedPrompt.trim();
    onUsePrompt(final);
  ...
  return (
    <>
          ...
          {filtered.map((p, i) => (
            <div key={p.id || i} className="rounded-xl p-3 group" style={{ background: 'var(--glass-bg)', border: '1px solid var(--border)' }}>
              ...
              <div className="flex gap-1">
                {/* Use button now opens the context modal */}
                <button onClick={() => openContextModal(p.title, p.prompt)}
      ...
      {/* Context / Additional Input Modal */}
      {contextModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setContextModal(null)}>
          ...
          >
            ...
            <div className="p-4 space-y-4">
              ...
              {/* Extra Context (code, instructions, etc.) */}
              <div>
